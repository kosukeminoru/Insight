...
//App for Validation
pub fn run(attempt: game::player::Attempt) {
    ...
    if proceed == true {
        match number.shot {
            1 => match attempt.first {
                Some(game::player::MouseResource {
                    first: _,
                    second: _,
                }) => game::player::shoot_player(&mut commands, &attempt.first.as_ref().unwrap()),
                None => println!("none"),
            },
            2 => match attempt.second {
                Some(game::player::MouseResource {
                    first: _,
                    second: _,
                }) => game::player::shoot_player(&mut commands, &attempt.second.as_ref().unwrap()),
                None => println!("none"),
            },
            3 => match attempt.third {
                Some(game::player::MouseResource {
                    first: _,
                    second: _,
                }) => game::player::shoot_player(&mut commands, &attempt.third.as_ref().unwrap()),
                None => println!("none"),
            },
            4 => match attempt.fourth {
                Some(game::player::MouseResource {
                    first: _,
                    second: _,
                }) => game::player::shoot_player(&mut commands, &attempt.fourth.as_ref().unwrap()),
                None => println!("none"),
            },
            5 => match attempt.fifth {
                Some(game::player::MouseResource {
                    first: _,
                    second: _,
                }) => game::player::shoot_player(&mut commands, &attempt.fifth.as_ref().unwrap()),
                None => println!("none"),
            },
            6 => exit.send(AppExit),
            _ => (),
...
pub fn vwin(query: Query<&Restitution>, mut exit: EventWriter<AppExit>) {
